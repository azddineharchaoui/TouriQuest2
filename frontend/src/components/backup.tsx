...
export function WelcomePage({ onGetStarted }: { onGetStarted: () => void }) {
  ...
  const [destinationsEmblaRef, destinationsEmblaApi] = useEmblaCarousel({ 
    loop: true, 
    slidesToScroll: 1,
    align: 'start',
    breakpoints: {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      testimonialsEmblaApi.scrollNext();
    }, 5000);
    return () => clearInterval(interval);
  ...
  return (
    <div className="min-h-screen from-slate-50 to-white">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          {heroImages.map((image, index) => (
            <div
              ...
              style={{
                backgroundColor: imageLoadErrors[index] ? '#1e40af' : 'transparent'
              }}
            >
              {!imageLoadErrors[index] && (
                <img
                  ...
                  loading="eager"
                  onError={() => {
                    console.log(`Primary image failed, trying fallback: ${image.url}`);
                    // Try fallback image
                    const fallbackImg = new Image();
                    fallbackImg.onload = () => {
                      // If fallback loads successfully, update the src
                      const imgElement = document.querySelector(`img[alt="${image.title}"]`) as HTMLImageElement;
                      if (imgElement) {
                        imgElement.src = image.fallback;
                      }
                    };
                    fallbackImg.onerror = () => {
                      console.log(`Fallback image also failed: ${image.fallback}`);
              ...
              {imageLoadErrors[index] && (
                <div className="w-full h-full flex items-center justify-center">
                  <div className="text-center text-black">
                    ...
                    <p className="text-lg opacity-80">{image.subtitle}</p>
                  </div>
                </div>
              )}
        ...
        <div className="relative z-10 text-center text-black max-w-6xl px-4">
          <div className="opacity-100 transform translate-y-0 transition-all duration-1000 delay-300">
            <Badge className="mb-6 bg-white/20 text-black border-white/30 backdrop-blur-sm px-4 py-2 text-sm">
          ...
          <p className="opacity-100 transform translate-y-0 transition-all duration-1000 delay-700 text-xl md:text-2xl mb-8 text-black/90 leading-relaxed max-w-4xl mx-auto">
          ...
          <Card className="opacity-100 transform translate-y-0 transition-all duration-1000 delay-900 p-6 bg-white/95 backdrop-blur-md max-w-4xl mx-auto mb-8 shadow-2xl rounded-2xl border-0">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Button size="lg" className="h-full bg-gradient-to-r from-blue-600 to-teal-600 hover:from-blue-700 hover:to-teal-700 text-black shadow-lg hover:shadow-xl transition-all duration-300 rounded-xl">
          ...
          <div className="opacity-100 transform translate-y-0 transition-all duration-1000 delay-1100 flex flex-col sm:flex-row gap-4 justify-center">
            <Button 
              ...
              className="bg-gradient-to-r from-blue-600 to-teal-600 hover:from-blue-700 hover:to-teal-700 text-black shadow-lg hover:shadow-xl transition-all duration-300 px-8 py-4 rounded-xl text-lg"
            >
              Start Your Journey
            ...
            <Button 
              size="lg" 
              variant="outline" 
              className="bg-white/10 hover:bg-white/20 text-black border-white/30 backdrop-blur-sm hover:border-white/50 transition-all duration-300 px-8 py-4 rounded-xl text-lg" 
              onClick={onGetStarted}
            >
        ...
        <div className="absolute top-8 right-8 z-10">
          <Badge className="bg-white/20 text-black border-white/30 backdrop-blur-sm px-4 py-2">
      ...
      <section className="py-20 px-4 bg-white">
        <div className="container mx-auto">
          ...
          <div className="animate-on-scroll opacity-0 transform translate-y-8 transition-all duration-1000 delay-300">
            <div className="embla overflow-hidden rounded-2xl" ref={featuresEmblaRef}>
              <div className="embla__container flex">
                {appFeatures.map((feature, index) => (
                  <div key={index} className="embla__slide flex-[0_0_100%] md:flex-[0_0_50%] lg:flex-[0_0_33.333%] px-4">
                    <Card className="h-full overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-all duration-500 group hover:-translate-y-2 bg-gradient-to-br from-white to-gray-50">
                      <div className="relative">
                        ...
                        <Badge className="absolute top-4 right-4 bg-white/20 text-black border-white/30 backdrop-blur-sm">
                          {feature.badge}
                        </Badge>
                      ...
                      <div className="p-6">
                        ...
                        <Button 
                          variant="outline" 
                          size="sm"
                          className="w-full group-hover:bg-blue-600 group-hover:text-black group-hover:border-blue-600 transition-all duration-300"
                        >
                          Learn More
            ...
            <div className="flex justify-center mt-8 space-x-4">
              <Button
                variant="outline"
                size="sm"
                onClick={() => featuresEmblaApi?.scrollPrev()}
                className="rounded-full w-12 h-12 p-0 hover:bg-blue-600 hover:text-black transition-all duration-300"
              >
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => featuresEmblaApi?.scrollNext()}
                className="rounded-full w-12 h-12 p-0 hover:bg-blue-600 hover:text-black transition-all duration-300"
              >
            ...
            <div className="embla overflow-hidden" ref={testimonialsEmblaRef}>
              <div className="embla__container flex">
                {testimonials.map((testimonial, index) => (
                  <div key={index} className="embla__slide flex-[0_0_100%] md:flex-[0_0_80%] lg:flex-[0_0_60%] px-4">
                    <Card className="p-8 border-0 shadow-xl bg-gradient-to-br from-white to-gray-50 text-center">
      ...
      {/* Enhanced Popular Destinations Carousel */}
      <section className="py-20 px-4 bg-gradient-to-br from-gray-50 to-white">
        <div className="container mx-auto">
          ...
          <div className="animate-on-scroll opacity-0 transform translate-y-8 transition-all duration-1000 delay-300">
            {/* Primary Grid Layout - Always Visible */}
              ...
              {popularDestinations.map((destination, index) => (
                <Card key={`destination-${index}`} className="overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-all duration-500 group hover:-translate-y-2 bg-white">
                  <div className="relative">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                    <Badge className="absolute top-4 left-4 bg-white/20 text-black border-white/30 backdrop-blur-sm">
                      <Star className="w-3 h-3 mr-1 fill-current" />
                      {destination.rating}
                    </Badge>
                    <Badge className="absolute top-4 right-4 bg-blue-500/20 text-black border-blue-400/30 backdrop-blur-sm">
                      {destination.experiences}
                    </Badge>
                    <div className="absolute bottom-4 left-4 right-4">
                      <h3 className="font-bold text-black text-xl mb-1">{destination.name}</h3>
                      <p className="text-black/90 text-lg">{destination.price}</p>
                    </div>
                  </div>
                  <div className="p-6">
                    <Button 
                      className="w-full bg-gradient-to-r from-blue-600 to-teal-600 hover:from-blue-700 hover:to-teal-700 text-black"
                    >
                      Explore Destination
                      <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
                    </Button>
            ...
            <div className="flex justify-center">
              <Button 
                variant="outline"
                className="px-8 py-3 hover:bg-blue-600 hover:text-black transition-all duration-300"
              >
      ...
      <section className="py-20 px-4 bg-gradient-to-br from-blue-600 via-blue-700 to-teal-600 relative overflow-hidden">
        ...
        <div className="container mx-auto text-center relative z-10">
          <div className="animate-on-scroll opacity-0 transform translate-y-8 transition-all duration-1000 max-w-4xl mx-auto">
            <Badge className="mb-6 bg-white/20 text-black border-white/30 backdrop-blur-sm px-4 py-2">
              <Sparkles className="w-4 h-4 mr-2" />
              Ready to Start?
            </Badge>
            ...
            <p className="text-xl md:text-2xl text-black/90 mb-12 leading-relaxed max-w-3xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center mb-12">
              ...
              <Button 
                size="lg" 
                variant="outline" 
                className="bg-white/10 hover:bg-white/20 text-black border-white/30 backdrop-blur-sm hover:border-white/50 transition-all duration-300 px-8 py-4 rounded-xl text-lg font-semibold" 
                onClick={onGetStarted}
              >
                <Play className="w-5 h-5 mr-2" />
                Watch Demo
