      ...
      {isOpen && (
        <div className="absolute top-12 right-0 bg-white border border-gray-300 rounded-lg shadow-xl w-80 max-h-96 overflow-y-auto">
          <div className="p-4 border-b bg-gray-50">
            <h3 className="font-semibold text-gray-800">Development Navigation</h3>
            <p className="text-sm text-gray-600">Current: {pages.find(p => p.state === currentState)?.label}</p>
          </div>
          <div className="p-2">
            {pages.map((page) => (
              <button
          ...
          <div className="p-4 border-t bg-gray-50 text-sm text-gray-600">
            💡 This panel is only for development - navigate easily without authentication!
          </div>
        </div>
      )}
    </div>
...
function AppContent() {
  ...
  if (appState === 'welcome') {
    return (
      <>
        {isDevelopmentMode && <DevNavigationPanel currentState={appState} onStateChange={setAppState} />}
  ...
  if (appState === 'property-detail') {
    return (
      <>
        {isDevelopmentMode && <DevNavigationPanel currentState={appState} onStateChange={setAppState} />}
        <Layout currentPage={appState} onNavigate={handleNavigation} onSearch={handleSearch}>
          <PropertyDetail 
  ...
  if (appState === 'audio-guide') {
    return (
      <>
        {isDevelopmentMode && <DevNavigationPanel currentState={appState} onStateChange={setAppState} />}
        <Layout currentPage={appState} onNavigate={handleNavigation} onSearch={handleSearch} showFooter={false}>
          <AudioGuide 
